...
public class BoardPanel extends JPanel {
    ...
    public static int arrR[] = {COLOR_MIN, COLOR_MIN, COLOR_MAX, COLOR_MAX, COLOR_MIN, 128, COLOR_MAX};
    public static int arrG[] = {COLOR_MAX, COLOR_MIN, 127, COLOR_MAX, COLOR_MAX, COLOR_MIN, COLOR_MIN};
    public static int arrB[] = {COLOR_MAX, COLOR_MAX, COLOR_MIN, COLOR_MIN, COLOR_MIN, 128, COLOR_MIN};
    /**
    ...
    public void paintComponent(Graphics g) {
        ...
        if (tetris.isPaused()) {
            ...
            for (int col = 0; col < type.getDimension(); col++) {
                for (int row = 0; row < type.getDimension(); row++) {
                    if (pieceRow + row >= 2 && type.isTile(col, row, rotation)) {
                        shineTile(type, (pieceCol + col) * TILE_SIZE, (pieceRow + row - HIDDEN_ROW_COUNT) * TILE_SIZE, g);
                    }
                }
            }
            ...
            if (iGlow > 125) {
                tetris.setGrow(false);
            }
            if (iGlow < 50) {
                tetris.setGrow(true);
            }

            tetris.setGlow(iGlow);

            base = new Color(base.getRed(), base.getGreen(), base.getBlue(), iGlow);
    ...
    private void shineTile(TileType type, int x, int y, Graphics g) {
        ...
        Color cSetColor = type.getBaseColor();
